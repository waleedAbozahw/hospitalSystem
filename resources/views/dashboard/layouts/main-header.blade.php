                ...
                <li class="">
                    <div class="dropdown  nav-itemd-none d-md-flex">
                        <a href="#" class="d-flex  nav-item nav-link pl-0 country-flag1" data-toggle="dropdown" aria-expanded="false">
                            @if (App::getLocale() == 'ar')
                            <span class="avatar country-Flag mr-0 align-self-center bg-transparent"><img src="{{URL::asset('dashboard/img/flags/egypt_flag.jpg')}}" alt="img"></span>
                            ...
                            <form method="POST" action="{{ route('logout.admin') }}">
                                ...
                                <form method="POST" action="{{ route('logout.doctor') }}">
                                    ...
                                    <form method="POST" action="{{ route('logout.ray_employee') }}">
                                        @elseif(auth('laporatory_employee')->check())
                                        <form method="POST" action="{{ route('logout.laporatory_employee') }}">
                                        ...
                                        this.closest('form').submit();"><i class="bx bx-slider-alt"></i>
                                                    تسجيل خروج
                                                </a>
                                            </form>
                    </div>
...
<script>
    ...
    Echo.private('create-invoice.{{ auth()->user()->id }}').listen('.create-invoice', (data) => {
        var newNotificationHtml = '<h4 class="notification-label mb-1">'+data.message+data.patient+'</h4><div class="notification-subtext">'+data.created_at+'</div>';
        new_message.show();
